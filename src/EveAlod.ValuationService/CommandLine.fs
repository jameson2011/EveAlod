...
module CommandLine=
    ...
    let addMongoServerArg =             addSingleOption dbServerArg "server" (sprintf "The MongoDB server name. Default: %s" ConfigurationDefaults.mongoServer)
    let getMongoServerValue app =       getStringOption dbServerArg app  |> Option.defaultValue ConfigurationDefaults.mongoServer
    let addMongoDbArg =                 addSingleOption dbNameArg dbNameArg (sprintf "The MongoDB DB name. Defaut: %s" ConfigurationDefaults.mongoDb)
    let getMongoDbValue app =           getStringOption dbNameArg app |> Option.defaultValue ConfigurationDefaults.mongoDb
    let addMongoCollectionArg =         addSingleOption dbCollectionArg "collection" (sprintf "The MongoDB DB collection name. Default: %s" ConfigurationDefaults.mongoCollection )
    let getMongoCollectionValue app =   getStringOption dbCollectionArg app |> Option.defaultValue ConfigurationDefaults.mongoCollection
    ...
    let getWebPortValue app =     
        ...
        | None  -> ConfigurationDefaults.webPort
        | Some x -> 
